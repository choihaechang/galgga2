...
<c:set var="memberID" value="${memberInfo.member_id }"/>
...
/* QA 작성  */
 function QAadd(){
	...
	function reviewReplAdd(z){
	
		var _reviewReplAdd = "reviewReplAdd" + z;
		document.getElementById(_reviewReplAdd).submit();
	}
	
	function reviewReplForm(z){
		
		var _reviewReplForm ="reviewReplForm"+z;
		var con = document.getElementById(_reviewReplForm);
	...
	function reviewReplList(z){
		var _reviewReplList = "reviewReplList"+z;
		var _lod_id = ${lod.lod_id};
		var $review_id = "review_id"+z;
		var _review_id = document.getElementById($review_id).value;
		
		
		$.ajax({
			type:"get",
			url:"${contextPath}/getData/reviewReplList.do",
			data : {lod_id : _lod_id, review_id : _review_id },
			async: false,
			success : function(data){
				...
				$(data.replList).each(function(){
					comments += '<strong>';
					comments += '<div class="reviewRepl">';
					comments += '<div style="margin-bottom:5px;"> 아이디 : '+this.member_id+'</div>';
					comments += '<div> 댓글 : '+this.repl_content+'</div>';
					comments += '</div>';
					comments += '</strong>';
	...
	function reviewRepl(data){
		
		$.each(List, function(index, value){
			alert("index : "+index+" value : "+value.m_id);
...
.myButton3, .myButton5:hover {
	background:linear-gradient(to bottom, #4b5578 5%, #6f88b3 100%);
	background-color:#4b5578;
}
.myButton3, .myButton5:active {
	position:relative;
	top:1px;
}
...
<input type="hidden" id="date_unit_name" name="unit_name" value="">
	...
	<div class="faq">
		  <div class="room" >
		  	...
		  	<button class="accordion" id="accodian${x}">
		  		<a href="${contextPath}/unitThumbnails.do?lod_id=${lod.lod_id}&unit_id=${item.unit_id}&unit_imgName=${unitImg[status.index].unit_imgName}" data-lightbox="image-1">
 ...
 <c:forEach var="re" items="${review}"> 
   ...
   <div style="font-size: 20px;"><strong>리뷰 제목 : </strong>${re.review_title }</div>
		...
		<div style="display:inline-block; width:710px; margin-bottom:15px;">
			<div style="margin: 0px 0px 15px 85px;  font-size: 17px; width: 480px; font-weight: 500; display:inline-block;">${re.review_content }</div>
			<div style="margin-left: 90%;">
				<c:if test="${re.m_id eq m_id }">
					<div style="display:inline-block;"><a style="cursor: pointer" onclick="	" class="myButton5">수정하기</a></div>
					<div style="display:inline-block;"><a style="cursor: pointer" onclick="	" class="myButton5">삭제하기</a></div>
				</c:if>
				<div style="display:inline-block;"><a style="cursor: pointer" onclick="return reviewReplForm(${z})" class="myButton5">댓글달기</a></div>
				<div style="display:inline-block;"><a style="cursor: pointer" onclick="return reviewReplList(${z})" class="myButton5">댓글보기</a></div>
			</div>
			
			
			<form action="${contextPath}/lods/reviewReplAdd.do" id="reviewReplAdd${z}" method="POST" >
				<div style="display:none;" id="reviewReplForm${z}">
					<div style="display:flex">
						<div class="revieReplCon">댓글 내용 :</div>
						<textarea style="resize:none; width:450px; height: 60px; margin-right:15px; border-radius: 8px;" name="repl_content"></textarea>
						<input type="hidden" name="m_id" value="${m_id }">
						<input type="hidden" name="review_id" id="review_id${z}" value="${re.review_id}">
						<input type="hidden" name="member_id" value="${memberInfo.member_id}">
						<input type="hidden" name="lod_id" value="${lod.lod_id}">
						<a style="cursor: pointer; margin:0px; padding-top: 22px;" onclick="return beforeReviewAdd(${z})" class="myButton5">댓글등록</a>
					</div>
				</div>
			</form>
			
			</div>
			<div class="kjs" id="kjs${z}">
